...
export FABRIC_CFG_PATH=/opt/gopath/src/github.com/hyperledger/fabric/build
echo "export FABRIC_CFG_PATH=/opt/gopath/src/github.com/hyperledger/fabric/build" >> ~/.profile
make gotools
make peer orderer peer-docker orderer-docker
curl -sSL http://bit.ly/2ysbOFE > bootstrap.sh
chmod +x bootstrap.sh
sudo ./bootstrap.sh -bs
sudo ./bootstrap.sh -ds
cd bin
NEW_PATH=$(pwd):$PATH
export PATH=NEW_PATH
echo "export PATH=$PATH" >> ~/.profile
echo "cd /opt/gopath/src/github.com/hyperledger/fabric" >> ~/.profile
EOF
